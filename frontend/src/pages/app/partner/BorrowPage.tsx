...
const BorrowPage = () => {
  ...
  const availableCredits = borrowerData?.data?.available ?? BigInt(0);
      ...
      <div className="space-y-4 sm:space-y-6">
            ...
            <div className="flex flex-col sm:flex-row sm:items-start gap-4">
              <Avatar className="h-12 w-12 sm:h-16 sm:w-16 self-center sm:self-start">
                <AvatarFallback className="text-sm sm:text-lg">
                  {borrowerData.data.metadata.name.charAt(0)}
                </AvatarFallback>
              </Avatar>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mb-4">
            Choose from our diverse range of loan pools with competitive rates
            and flexible terms. Each pool is designed to meet specific funding
            needs.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
        ...
        {selectedPool && (
          <BorrowModal
            ...
            pool={selectedPool}
            availableCredits={Number(formatEther(availableCredits))}
          />
        )}
      </div>
    );
