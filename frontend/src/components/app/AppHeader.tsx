...
import { Menu, Wallet, LogOut, Settings, ExternalLink, Droplets, ChevronDown } from "lucide-react";
    ...
    >
      <div className="max-w-[1248px] w-full flex items-center justify-between px-3 md:px-6 h-full mx-auto">
        <div className="flex items-center space-x-3">
          ...
          <h1 className="text-base md:text-2xl font-bold text-foreground truncate max-w-[120px] sm:max-w-none">{getPageTitle()}</h1>
        </div>
        ...
        <div className="flex items-center space-x-1 md:space-x-3">
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56 z-50">
              ...
              <DropdownMenuItem onClick={() => window.open('https://scan.test2.btcs.network/faucet', '_blank', 'noopener,noreferrer')}>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button className="flex items-center space-x-1 md:space-x-2 text-xs md:text-sm px-2 md:px-4 py-1 md:py-2">
                  <span className="hidden sm:inline">{formatAddress(address, false)}</span>
